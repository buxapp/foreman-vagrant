...
if [ "$PUPPETLABS_RELEASE_INSTALLED" == "" ]; then
    ...
    rm -rf /tmp/puppetlabs-release-wheezy.deb 1>/dev/null 2>&1
    wget -P /tmp http://apt.puppetlabs.com/puppetlabs-release-wheezy.deb 1>/dev/null 2>&1
    sudo dpkg -i /tmp/puppetlabs-release-wheezy.deb 1>/dev/null 2>&1
